...
function findBestBet(loss,bet,multiplier){
    ...
    if(loss>0){//increase multiplier
        currentM = multiplier*BASE.MULTI_BOOST
        currentB =  (loss/(currentM-1))  
    }
...
function maxLossBudget(bet,multiplier,maxLoseStreak){
    ...
    for(x=1;x<maxLoseStreak;x++){
        [newbet,newmulti] = findBestBet(loss,0,multiplier)  
        loss += newbet   
    }
    return loss
}
 ...
 function newDay(bet,multiplier){
    ...
    let dayStat = `day ${BASE.prevDay} total games: ${BASE.gameNumber} multiplier ${multiplier} loseStreak ${BASE.loseStreaks.length} maxLoseStreak ${maxLoseStreak} maxloss ${loss} lose count ${BASE.loseStreaks.flat().length}  winStreak: ${BASE.winStreaks.length} win count: ${BASE.winStreaks.flat().length} BASE.profit: ${BASE.profit}`
    console.log(dayStat)
 ...
 for (const element of data) {
    if(BASE.prevDay != element[0]){
     ...
     [currentBet,newmulti] = findBestBet(prevLoss,bet,multiplier)  

    
   // if(BASE.gameNumber >= offset){

        if(element[1]>=newmulti){
           ...
           if(currentLoseStreak.length>0) BASE.loseStreaks.push(currentLoseStreak)
            ...
            //    console.log(`%c prevLoss recovered ${prevLoss} BASE.gameNumber: ${BASE.gameNumber} BASE.profit: ${BASE.profit}`,'color: green')
                ...
                let potentialprofit =currentBet*(Math.pow(newmulti,currentWinStreak.length)-1)               
            ...
            if(currentWinStreak.length==streakLength){
                BASE.profit +=bet*(Math.pow(newmulti,currentWinStreak.length)-1)     
